...
void Player::createPipeline(){
    if (!m_pipeline) {
//        m_pipeline = QGst::ElementFactory::make("v4l2src").dynamicCast<QGst::Pipeline>();
        if(pipelineSet){
            ...
            qCritical() << "Pipeline description was not set";
            QCoreApplication::exit(-100);
        }
    }else{
        qWarning() << "Pipeline already created";
    }
/*
Yes, this is a Qt bug. QWidget::paintEngine is for Qt core internal use only but QMediaPlayer seems to be using it,
possibly indirectly via a WA_PaintOnScreen flag being set at some point inside the library.
For what it's worth, you are not the only person to notice this issue,
and if the player works the warning can be ignored fairly safely.
Future Qt updates will likely resolve this issue over time.
 */
}

void Player::setPipelineDescription(QString pipeDescr){
